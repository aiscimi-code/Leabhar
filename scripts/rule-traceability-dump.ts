...
const INGEST_SOURCES = [
  'finance-act-2024', 'vatca-2010', 'vatca-2010-sch2', 'vatca-2010-sch3', 'rct-tca530',
...
async function run(): Promise<void> {
  ...
  for (const [cmd, list] of [['ingest', INGEST_SOURCES], ['extract', EXTRACT_SOURCES]] as const) {
    for (const source of list) {
      const args = [cmd, '--source', source];
      silence();
