...
| 9 | `vat-rates/current-vat-rates.md` + `rates.json` + `schedule-moves-2025-2026.md` | Revenue current VAT rates table | https://www.revenue.ie/en/vat/vat-rates/search-vat-rates/current-vat-rates.aspx | Complete (2020-2026), retrieved 2026-09-18. `rates.json` is the same data machine-readable, keyed by `effective_from`. **Decided in issue #133: stays reference-only, deliberately never wired into `tax_rates`** — none of these three files carries a source hash, so none backs a deterministic rule or drives live financial config. The app's `tax_rates` table is instead synced from the source-hash-verified `irish_tax_rules` rate facts (`src/domain/rules/taxRateSync.ts`, `npm run cli:rules -- sync-tax-rates`), gated on human review. |
