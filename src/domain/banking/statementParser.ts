...
const COLUMN_PATTERNS: Array<{ field: DomainField; patterns: RegExp[] }> = [
  ...
  { field: 'base_amount', patterns: [/^total\s*amount$/i, /^settle(d)?\s*amount$/i, /^charged\s*amount$/i, /^euro?\s*amount$/i, /^base\s*amount$/i, /^settle(d)?\s*value$/i, /^account\s*amount$/i, /^payment\s*amount$/i] },
